...
5. Нажимаем кнопку - "Start Bot" в окошке Контроллера
5.1 Убеждаемся, что окно А-бота появилось на экране, а в в окошке Контроллере справа от кнопки "Connect" появились циферки например :0x170179
6. Для проверки жмем кнопку "Call HNDL" - Окошка А-бота которую открыли ранее, должно затрястись
7. Нажимаем кнопку "Start trade" в окошке Контроллера, ждем некоторое время
