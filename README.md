...
3. Кнопка "Connetct" - пытается подключиться к выбранному в пункте 1 боту, если бот не запущен - всплывет предупреждение
4. Кнопка "CallHNDL" - Шатает окно А-Бота к которому подключен DeltaController
